...
struct AztecEncoderTests {
    ...
    func encodes_large_binary_data_requiring_12_layers() throws {
        ...
        // 366 bytes of binary data requires a full symbol with 12-13 layers.
        ...
        // Layer count may vary slightly based on encoding efficiency
        #expect(result.configuration.layerCount >= 12 && result.configuration.layerCount <= 14,
                "366 bytes should use 12-14 layers, got \(result.configuration.layerCount)")
        #expect(result.configuration.isCompact == false, "Large data requires full symbol")
        #expect(result.symbol.size > 0, "Should produce a valid symbol")
    }
