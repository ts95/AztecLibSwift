...
public struct AztecEncoder: Sendable {
    ...
    private static func encodeData(_ dataBits: BitBuffer, options: Options) throws -> AztecSymbol {
        ...
        // Pad or truncate to match expected data codeword count
        while dataCodewords.count < configuration.dataCodewordCount {
            // Pad with zeros (will be stuffed appropriately)
            dataCodewords.append(0)
        }
        if dataCodewords.count > configuration.dataCodewordCount {
            dataCodewords = Array(dataCodewords.prefix(configuration.dataCodewordCount))
        }
